...
function formatWeather(data) {
  ...
  var saveData = {
    ...
    sunRise: rise.getFullYear() + '-' + (rise.getMonth() + 1) + '-' + rise.getDate() + ' ' + rise.getHours() + ':' + rise.getMinutes() + ':' + rise.getSeconds(),
    sunSet: set.getFullYear() + '-' + (set.getMonth() + 1) + '-' + set.getDate() + ' ' + set.getHours() + ':' + set.getMinutes() + ':' + set.getSeconds(),
    clouds: data.clouds.all,
    weatherCondition: data.weather[0].main,
    weatherDescrip: data.weather[0].description,
    icon: parseInt(data.weather[0].icon.replace("d|n", ""))
