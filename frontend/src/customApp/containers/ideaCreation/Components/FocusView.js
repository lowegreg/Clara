...
export class FocusView extends React.Component {
    constructor(props) {
        super(props);
    ...
    async getData() {
        try {
            let response = await fetch(UserProfile.getDatabase() + 'findPost' , {
                ...
                body: JSON.stringify({
                    empID: UserProfile.getID(), // pass empID to tell whether or not the user has liked/followed the idea
    ...
    render() {
        ...
        if (UserProfile.getID() === this.state.empID) {
		...
		if (this.state.status !== 'inReview' && this.state.status !== 'inProgress' && this.state.status !== 'Completed') {
			if (UserProfile.getID() === this.state.empID || UserProfile.getView() === 'admin' || UserProfile.getView() === 'lab') {
				menuButton = button;
			}
